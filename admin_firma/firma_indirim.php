        ...
        <div id="content">
            <div class="inner" style="min-height:1200px;">
                ...
                <div class="col-lg-12">
		                    ...
		                    <form action="" method="post" enctype="multipart/form-data">
		                    	<div class="panel panel-default">
			                        <div class="panel-heading">
			                        	<b>Ürün İsim:</b> <?php echo $row['urun_isim']; ?>
			                         	<b style="float: right;">Fiyat: <?php echo $row['urun_fiyat']; ?> TL</b>
			                        </div>
			                        <div class="panel-body">
			                            <input class="form-control" name="indirim_urun_isim" type="text" value="<?php echo $row['urun_isim']; ?>"required>                                          
		                                <input class="form-control" name="indirim_tutar" placeholder="İndirim tutarını sayı olarak yazınız" required>		                                
			                        </div>
			                        <div class="panel-footer">
			                           <button type="submit" name="urun_indirim" class="btn btn-primary btn-sm">İndirim Uygula</button>
			                        </div>
			                    </div>
                    ...
                    ?>                                               
                    <?php                  
	                    if(isset($_POST['urun_indirim'])){
							$indirim_firma= $b; 
							$indirim_urun_isim =$_POST["indirim_urun_isim"];					
							$indirim_tutar = $_POST["indirim_tutar"]; 

								if($indirim_tutar<$fiyat){	                        
									$ekle=$db->prepare("insert into indirim set indirim_firma=?,indirim_urun_isim=?,indirim_tutar=?");
									$ekle->execute(array($indirim_firma,$indirim_urun_isim,$indirim_tutar));
									echo "<script type='text/javascript'>alert('İndirim Başarıyla Eklendi');</script>";
								}
								else if ($indirim_tutar<= 0) {
									echo "<script type='text/javascript'>alert('İndirim Tutarı 0'dan küçük olamaz);</script>";
								}
								else {
			                    	echo "<script type='text/javascript'>alert('Ürün Fiyatından Fazla Değer Girdiniz');</script>";
			                    }							
	                    }                   							                       			
				?> 						   							
				</div>
