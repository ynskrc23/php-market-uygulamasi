...
<body>
	...
	<?php
         $_il=$_POST['uye_il'];
         $_ilce=$_POST['uye_ilce'];
         $il = $db->query("SELECT * FROM muh_iller where id='".$_il."'")->fetchAll(PDO::FETCH_ASSOC);
         $ilce = $db->query("SELECT * FROM muh_ilceler where id='".$_ilce."'")->fetchAll(PDO::FETCH_ASSOC);
			...
			<div class="panel-group category-products" id="accordian">
				...
				<div class="col-sm-9 padding-right">
					<div class="features_items"><!--features_items-->
						<h2 class="title text-center"> <?php foreach ($il as $key => $value) {echo $value['baslik'];}?> / <?php foreach ($ilce as $key => $value) {echo $value['baslik'];}?> </h2>
						<h2 class="title text-center">HAFTANIN ÜRÜNLERİ </h2>
						<?php $konumfirma=$db->query("select  * from uye where uye_il='".$_il."' and uye_tur='Firma' ")->fetchAll(PDO::FETCH_ASSOC); ?>
						<?php 
